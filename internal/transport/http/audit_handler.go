...
func (h *Handler) ListTenantAuditEvents(w http.ResponseWriter, r *http.Request) {
	...
	sessionTenantID := GetTenantID(r.Context())
	if sessionTenantID == "" {
		respondError(w, http.StatusForbidden, "tenant context required for this endpoint")
		return
	}
	...
	// Authorization: Check if user can view audit logs for this tenant (Tenant Admin/Owner ONLY)
	fmt.Printf("DEBUG ListTenantAuditEvents: User=%s Tenant=%s Requesting Tenant Audit\n", userID, tenantID)
	hasPerm, _ := h.authzService.HasPermission(r.Context(), userID, authz.ScopeTenant, &tenantID, authz.PermTenantViewAudit)
	fmt.Printf("DEBUG ListTenantAuditEvents: HasPermission=%v\n", hasPerm)

	if !hasPerm {
		respondError(w, http.StatusForbidden, "permission tenant:view_audit required")
		return
	}
	...
	if err != nil {
		fmt.Printf("ERROR ListTenantAuditEvents: repo.List failed: %v\n", err)
...
func (h *Handler) ListPlatformAuditEvents(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("DEBUG ListPlatformAuditEvents: User=%s Requesting Platform Audit\n", userID)
	isPlatformAdmin, _ := h.authzService.HasPermission(r.Context(), userID, authz.ScopePlatform, nil, authz.PermPlatformViewAudit)
	fmt.Printf("DEBUG ListPlatformAuditEvents: isPlatformAdmin=%v\n", isPlatformAdmin)
	if !isPlatformAdmin {
		respondError(w, http.StatusForbidden, "platform admin access required")
		return
	}

	// Intent-based flow: Traditional cross-tenant listing is PROHIBITED.
	// Platform Admin MUST list platform-level logs only,
	// or use the Audit Query flow for tenant logs.
	...
	events, total, err := h.auditRepo.List(r.Context(), filter)
	if err != nil {
		fmt.Printf("ERROR ListPlatformAuditEvents: repo.List failed: %v\n", err)
		respondError(w, http.StatusInternalServerError, "failed to list audit events")
		return
	}
...
func (h *Handler) CreateAuditQuery(w http.ResponseWriter, r *http.Request) {
	...
	// Validation: Tenant exists and not platform scope
...
func (h *Handler) GetAuditQueryResult(w http.ResponseWriter, r *http.Request) {
	...
	filter := audit.Filter{
		...
		StartDate: &start,
		EndDate:   &end,
		Limit:     100, // Fixed limit for scoped view
	}
